...
namespace :db do
  namespace :seed do
    ...
    task :run_updates, [:file_path] => :environment do |_t, args|
      filepath = args.fetch(:file_path, nil)
      if filepath && File.exist?(filepath)
        Rake::Task['db:seed:import_books'].invoke(filepath)
        Rake::Task['db:seed:import_catalogs'].invoke(filepath)
        Rake::Task['db:seed:import_reviews'].invoke(filepath)
        Rake::Task['db:seed:import_series'].invoke(filepath)
        Rake::Task['db:seed:import_subjects'].invoke(filepath)
      end
    end
    desc 'Import books to database'
    task :import_books, [:filepath] => :environment do |_t, args|
      BOOK_DATA = args.fetch(:filepath, nil)

      # If there is no file, create an empty node
      # so we don't throw an error when
      doc = if BOOK_DATA
              File.open(BOOK_DATA, 'rb:UTF-16le') { |f| Nokogiri::XML(f) }
            else
              Nokogiri::XML('')
            end
      ...
      doc.xpath('//record').map do |node|
        next unless node.xpath('title').text.present?

        book = Book.find_by(book_id: node.xpath('book_id').text)
        if book.nil?
          book = Book.new(book_id: node.xpath('book_id').text)
          new_book = true
        end
        book.tap do |b|
          ...
          b.series_id = node.xpath('series/series_id').text
          ...
          b.hotweight = '1'
          b.newsweight = '1'
          b.catalog_id = node.xpath('catalog').text
        end # tap
        if book.save
          if !new_book.nil?
      ...
      puts 'books updated: ' + (updated_ids.length + created_ids.length).to_s + "\n"
      puts 'books errored: ' + error_ids.length.to_s
