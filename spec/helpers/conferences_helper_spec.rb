    ...
    describe "dates formatter" do
      it "returns the date range within the same year" do
        if today.strftime("%Y") == next_month.strftime("%Y")
          expect(helper.dates(today,
                              next_month)).to eq("#{today.strftime('%b %d')} - #{next_month.strftime('%b %d, %Y')}")
        else
          expect(helper.dates(today,
                              next_month)).to eq("#{today.strftime('%b %d, %Y')} - #{next_month.strftime('%b %d, %Y')}")
        end
      end
      it "returns single date with year" do
        expect(helper.dates(today)).to eq(today.strftime("%b %d, %Y").to_s)
      end
