...
RSpec.describe EventsHelper, type: :helper do
  ...
  describe "date_range" do
    it "returns single day format" do
      ...
      eql([start_date.strftime("%a %h"), "#{start_date.strftime('%d').to_i.ordinalize},",
           start_date.strftime("%l:%M %P")].join(" "))
    end
    it "returns multi day format" do
      helper.date_range(start_date, DateTime.tomorrow).should
      eql("#{[start_date.strftime('%a %b'), "#{start_date.strftime('%d').to_i.ordinalize},",
              start_date.strftime('%l:%M %P')].join(' ')}
        - #{[end_date.strftime('%a %h'), "#{end_date.strftime('%d').to_i.ordinalize},",
             end_date.strftime('%l:%M %P')].join(' ')}")
    end
  end
end
