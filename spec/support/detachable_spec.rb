...
RSpec.shared_examples "detachable", type: :request do
  describe "GET /admin/#{described_class.to_s.underscore.pluralize}/:id/detach" do
    ...
    before(:each) do
      ...
      factory_model.image.attach(image) if %i[brochure event series person webpage
                                              oabook].include?(described_class)
      factory_model.pdf.attach(pdf) if %i[special_offer subject webpage oabook].include?(described_class)
    ...
    it "detaches file from #{described_class.to_s.underscore.pluralize}" do
      ...
      expect(response.body).to include("charles.jpg") if %i[book brochure event series
                                                            person].include?(described_class)
      expect(response.body).to include("guidelines.pdf") if %i[special_offer subject
                                                               oabook].include?(described_class)

      get "#{admin_show_page}/detach?field=image" if %i[brochure event series person
                                                        oabook].include?(described_class)
      get "#{admin_show_page}/detach?field=pdf" if %i[special_offer subject oabook].include?(described_class)
      get "#{admin_show_page}/detach?field=cover_image" if [:book].include?(described_class)

      expect(response).to render_template(:show)

      expect(response.body).to_not include("charles.jpg") if %i[book brochure event series person
                                                                oabook].include?(described_class)
      expect(response.body).to_not include("guidelines.pdf") if %i[special_offer subject
                                                                   oabook].include?(described_class)
    end
  end
end
