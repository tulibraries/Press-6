...
RSpec.shared_examples "detachable", type: :request do
  ...
  describe "GET /admin/#{described_class.to_s.underscore.pluralize}/:id/detach" do
    ...
    before(:each) {
      ...
      factory_model.image.attach(image) if [:brochure, :event, :series, :person, :webpage, :oabook].include?(described_class)
    ...
    it "detaches file from #{described_class.to_s.underscore.pluralize}" do
      ...
      expect(response.body).to include("charles.jpg") if [:book, :brochure, :event, :series, :person].include?(described_class)
      expect(response.body).to include("guidelines.pdf") if [:special_offer, :subject, :oabook].include?(described_class)

      get "#{admin_show_page}/detach?field=image" if [:brochure, :event, :series, :person, :oabook].include?(described_class)
      ...
      expect(response.body).to_not include("charles.jpg") if [:book, :brochure, :event, :series, :person, :oabook].include?(described_class)
