...
RSpec.shared_examples "detachable", type: :request do
  ...
  describe "GET /admin/#{described_class.to_s.underscore.pluralize}/:id/detach" do
    ...
    it "detaches file from #{described_class.to_s.underscore.pluralize}" do
      ...
      get "#{show_page}/detach?field=image" if [:brochure, :series, :person, :oabook].include?(described_class)
      get "#{show_page}/detach?field=pdf" if [:special_offer, :subject, :oabook].include?(described_class)
      get "#{show_page}/detach?field=cover_image" if [:book].include?(described_class)
